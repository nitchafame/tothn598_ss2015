...
void main() {
    ...
    pct.r = exponentialInOut( abs(fract(t)*2.0-1.) );
    pct.g = smoothstep(5.0, 0.05,sin(st.y *PI)+2.0);
    pct.b = smoothstep(2.5, 0.05,sin(st.y *PI)+1.5);

    color = mix(colorA, colorC, pct);

    // vec3 sunrise = mix(colorA, colorC, pct);
    // vec3 sunset = mix(colorB, colorA, pct);
