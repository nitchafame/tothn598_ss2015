// Author_ Nitcha Tothong 
// http://nitchafa.me


#ifdef GL_ES
precision mediump float;
#endif

uniform sampler2D u_tex0;
uniform vec2 u_tex0Resolution;

uniform vec2 u_resolution;
uniform vec2 u_mouse;
uniform float u_time;

float random (vec2 st) { 
    return fract(sin(dot(st.xy,
                         vec2(12.9898,78.233)))* 
        43758.5453123);
}

// Based on Morgan McGuire @morgan3d
// https://www.shadertoy.com/view/4dS3Wd
float noise (in vec2 _st) {
    vec2 i = floor(_st);
    vec2 f = fract(_st);

    // Four corners in 2D of a tile
    float a = random(i);
    float b = random(i + vec2(1.0, 0.0));
    float c = random(i + vec2(0.0, 1.0));
    float d = random(i + vec2(1.0, 1.0));

    vec2 u = f * f * (3.0 - 2.0 * f);

    return mix(a, b, u.x) + 
            (c - a)* u.y * (1.0 - u.x) + 
            (d - b) * u.x * u.y;
}

vec3 mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec2 mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 permute(vec3 x) { return mod289(((x*34.0)+1.0)*x); }

float snoise(vec2 v) {
    const vec4 C = vec4(0.211324865405187,  // (3.0-sqrt(3.0))/6.0
                        0.366025403784439,  // 0.5*(sqrt(3.0)-1.0)
                        -0.577350269189626,  // -1.0 + 2.0 * C.x
                        0.024390243902439); // 1.0 / 41.0
    vec2 i  = floor(v + dot(v, C.yy) );
    vec2 x0 = v -   i + dot(i, C.xx);
    vec2 i1;
    i1 = (x0.x > x0.y) ? vec2(1.0, 0.0) : vec2(0.0, 1.0);
    vec4 x12 = x0.xyxy + C.xxzz;
    x12.xy -= i1;
    i = mod289(i); // Avoid truncation effects in permutation
    vec3 p = permute( permute( i.y + vec3(0.0, i1.y, 1.0 ))
        + i.x + vec3(0.0, i1.x, 1.0 ));

    vec3 m = max(0.5 - vec3(dot(x0,x0), dot(x12.xy,x12.xy), dot(x12.zw,x12.zw)), 0.0);
    m = m*m ;
    m = m*m ;
    vec3 x = 2.0 * fract(p * C.www) - 1.0;
    vec3 h = abs(x) - 0.5;
    vec3 ox = floor(x + 0.5);
    vec3 a0 = x - ox;
    m *= 1.79284291400159 - 0.85373472095314 * ( a0*a0 + h*h );
    vec3 g;
    g.x  = a0.x  * x0.x  + h.x  * x0.y;
    g.yz = a0.yz * x12.xz + h.yz * x12.yw;
    return 130.0 * dot(m, g);
}

// Based on Golan Levin 
// http://www.flong.com/texts/code/shapers_poly/
float doubleCubicSeat (float x, float a, float b){
  float y = 0.0;
  if (x <= a){
    y = b - b*pow(1.0-x/a, 3.0);
  } else {
    y = b + (1.0-b)*pow((x-a)/(1.0-a), 3.0);
  }
  return y;
}

#define NUM_OCTAVES 5

float fbm ( in vec2 _st) {
    float v = 0.0;
    float a = 0.5;
    vec2 shift = vec2(100.0);
    // Rotate to reduce axial bias
    mat2 rot = mat2(cos(0.5), sin(0.5), 
                    -sin(0.5), cos(0.50));
    for (int i = 0; i < NUM_OCTAVES; ++i) {
        v += a * noise(_st);
        _st = rot * _st * 2.0 + shift;
        a *= 0.5;
    }
    return v;
}

float pcurve( float x, float a, float b )
{
    float k = pow(a+b,a+b) / (pow(a,a)*pow(b,b));
    return k * pow( x, a ) * pow( 1.0-x, b );
}



mat2 rotate2d(float angle){
    return mat2(cos(angle),-sin(angle),
                sin(angle),cos(angle));
}


float fractCircles(in vec2 pos, float size) {
  float scale = 6.0;
  pos *= scale;
  float pct = fract(length(pos/size));
  return  step(0.35, pct) - step(0.5, pct);
}

void main() {
    vec2 st = gl_FragCoord.xy/u_resolution.xy;
    //st.x += 0.6;
    vec3 pct = vec3(0.0);
    vec4 color = vec4(0.0);
    float offset = float (0.0);
    
    vec2 translate = vec2(doubleCubicSeat(3.0,u_time-9.0,6.0),sin(u_time*4.0+6.0*st));
    st += translate*0.02;

    //vec2 pos = st.xy * pow(1.0,2.0);
    vec2 pos = st.xy * pow(2.0,2.0);
    float pattern = pos.y;
    float DF = 0.0;
    float a = 0.0;
    vec2 vel = vec2(u_time*.05);
    DF += snoise(pos+vel)*0.2+ 0.4;

    // Add a random position
    a = snoise(pos*vec2(cos(u_time*0.5),sin(u_time*0.6))*0.1)*3.1415;
    vel = vec2(cos(a),sin(a));
    DF += snoise(pos+vel)*0.15+0.25;

    offset = smoothstep(0.5,.75,fract(DF));
    
    color = texture2D (u_tex0, st + offset);
    color *= color * offset;
    
    color *= texture2D (u_tex0, st + offset);

    //Add psychedelic color layers
    color.r = texture2D(u_tex0,st+offset).r;
    color.g = texture2D(u_tex0,st+pow(vec2(offset),vec2(2.0))).g;
    color.b = texture2D(u_tex0,st+pow(vec2(offset),vec2(3.0))).b;
   
    //Add noise texture
    pos = rotate2d( noise(pos) ) * pos;
    pattern = fractCircles(pos + offset,0.0005);
    color += color*pattern;
 

    gl_FragColor = vec4 (color);
}